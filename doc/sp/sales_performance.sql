...
call procPartnerPC_Sales_Performance_get('pettester@peteasy.kr');
DELIMITER $$
...
BEGIN
   ...
   */
    ...
    CASE 
		...
		THEN
        BEGIN
			SET @order = 'ORDER BY A.payment_type ASC, A.reservationDate DESC';
			SET @where = IF(dataWhere != '', CONCAT(' AND A.payment_type = ', dataWhere), '');
		END;
		WHEN dataType = 'service'
		THEN
        BEGIN
			SET @order = 'ORDER BY A.service DESC';
            SET @where = CONCAT(' AND A.payment_type = ','미용');
			SET @where = CONCAT(@where, IF(dataWhere != '', CONCAT(' AND A.service = ', dataWhere), ''));
		END;
		WHEN dataType = 'artist'
		THEN
        BEGIN
			SET @order = 'ORDER BY A.worker DESC';
			SET @where = IF(dataWhere != '', CONCAT(' AND A.payment_type = ', dataWhere), '');
		END;
        ...
        BEGIN
			...
			SET @where = IF(dataWhere != '', CONCAT(' AND A.payment_type = ', dataWhere), '');
		END;
        ELSE SET @order = 'ORDER BY A.payment_type ASC, A.reservationDate DESC';
    END CASE;
		...
		FROM (
			...
			WHERE ppl.is_delete = '2'
				...
				AND DATE_FORMAT(CONCAT(pr.check_in_date,' ',pr.check_in_time), '%Y-%m-%d %H:%i') 
					BETWEEN DATE_FORMAT(CONCAT(?, ' 00:00'), '%Y-%m-%d %H:%i') 
						 AND DATE_FORMAT(CONCAT(?, ' 00:00'), '%Y-%m-%d %H:%i')
		) AS A WHERE 1=1 ", @ORD_STR);#AND AA.ymdhm = BB.ymdhm
	...
	SELECT *
		FROM (
			SELECT 
			    ...
			    , payment_log_seq AS pls
			    , pl.pet_seq AS b_pet_seq
				, if(worker = dataPartnerID, '대표', worker) AS worker
				, (
					case
						when pay_type = 'card' then '앱-선결제'
						when pay_type = 'bank' then '앱-선결제'
			...
			WHERE is_cancel = 0
				...
				AND DATE_FORMAT( CONCAT( year,'-',month,'-',day,' ',hour,':',IFNULL(minute,'00'),':00' ), '%Y-%m-%d %H:%i' ) 
					BETWEEN DATE_FORMAT(CONCAT(dataStDate, ' 00:00'), '%Y-%m-%d %H:%i') 
						 AND DATE_FORMAT(CONCAT(dataFiDate, ' 23:59'), '%Y-%m-%d %H:%i') 
			...
			WHERE hpl.is_delete = '2'
				AND hpl.is_no_show = '2'
				AND hpl.data_delete = '0'
				AND hpl.artist_id = dataPartnerID
				AND DATE_FORMAT(CONCAT(hr.check_in_date,' ',hr.check_in_time), '%Y-%m-%d %H:%i') 
					BETWEEN DATE_FORMAT(CONCAT(dataStDate, ' 00:00'), '%Y-%m-%d %H:%i') 
						 AND DATE_FORMAT(CONCAT(dataFiDate, ' 00:00'), '%Y-%m-%d %H:%i')
			...
			WHERE ppl.is_delete = '2'
				AND ppl.is_no_show = '2'
				AND ppl.data_delete = '0'
				AND ppl.artist_id = dataPartnerID
				AND DATE_FORMAT(CONCAT(pr.check_in_date,' ',pr.check_in_time), '%Y-%m-%d %H:%i') 
					BETWEEN DATE_FORMAT(CONCAT(dataStDate, ' 00:00'), '%Y-%m-%d %H:%i') 
						 AND DATE_FORMAT(CONCAT(dataFiDate, ' 00:00'), '%Y-%m-%d %H:%i')
