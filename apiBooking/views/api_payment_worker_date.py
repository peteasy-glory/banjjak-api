...
class TDateHotel(TAPIBookingBase):
    def getInfo(self, payment_idx, *args):
        try:
            pass
        except Exception as err:
    ...
    def modifyInfo(self, *args):
        try:
            body = {}
            if args[1] == 'PUT':
                value, rows, columns = self.db.resultDBQuery(PROC_BEAUTY_BOOKING_DATE_HOTEL_PUT % (args[0]["order_num"]
                                                            , args[0]["check_in_date"], args[0]["check_out_date"]), QUERY_DB)
                if value is not None:
                    body = self.queryDataToDic(value, rows, columns)
                return 0, "success", body
            return - 1, "undefined method", body
