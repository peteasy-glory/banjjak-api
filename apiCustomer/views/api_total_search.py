...
class TTotalSearch(TAPICustomerBase):
    ...
    def getInfo(self, payment_id, *args):
        try:
            count = []
            if args[0] == "beauty":
                value, rows, columns = self.db.resultDBQuery(PROC_CUSTOMER_BEAUTY_TOTAL_SEARCH_GET % (payment_id,), QUERY_DB)
            elif args[0] == "hotel":
                value, rows, columns = self.db.resultDBQuery(PROC_CUSTOMER_HOTEL_TOTAL_SEARCH_GET % (payment_id,),QUERY_DB)
            elif args[0] == "kinder":
                value, rows, columns = self.db.resultDBQuery(PROC_CUSTOMER_KINDER_TOTAL_SEARCH_GET % (payment_id,),QUERY_DB)
            elif args[0] == "people":
